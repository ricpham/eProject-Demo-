...
$(document).ready(function () {
  ...
  $("input[type=checkbox]").click(
    ...
    function () {
    let check = $("#check-brands:checked") 
      .map(function (e) { 
        return e.val();
      })
      .toArray()
    ...
    function isMatch(items) {
      return check.search(item.pdbrand) >=0
    }
  
    let subdata;
    let subdata = check.length == 0 ? data : data.filter(isMatch);
    showImage(subdata);
  });
});
